...
const CartPanelComponent = () => {
    ...
    // Dynamic progress bar color "heating up" logic
    const getProgressColor = (percent: number) => {
        if (percent >= 100) return "[&>div]:bg-red-600 shadow-[0_0_10px_rgba(220,38,38,0.5)]"; // Overheat
        if (percent >= 90) return "[&>div]:bg-red-500"; // Critical
        if (percent >= 75) return "[&>div]:bg-orange-500"; // Hot
        if (percent >= 50) return "[&>div]:bg-amber-400"; // Warm
        return "[&>div]:bg-emerald-500"; // Cool
    };

    const getButtonColor = (percent: number) => {
        if (percent >= 100) return "bg-red-600 hover:bg-red-700 shadow-red-600/20";
        if (percent >= 90) return "bg-red-500 hover:bg-red-600 shadow-red-500/20";
        if (percent >= 75) return "bg-orange-500 hover:bg-orange-600 shadow-orange-500/20";
        if (percent >= 50) return "bg-amber-500 hover:bg-amber-600 shadow-amber-500/20";
        return "bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/20";
    };

    const getStatusText = (percent: number) => {
        if (percent >= 100) return "Orçamento Excedido";
        if (percent >= 90) return "Cuidado, quase lá!";
        if (percent >= 75) return "Atenção ao limite";
            ...
            <div className={cn(
                ...
                >
                    ...
                    <div
                        ...
                        className="relative flex items-center justify-between p-4 cursor-pointer group pt-6"
                    >
                        {/* Center Chevron Indicator */}
                        <div className="absolute top-1.5 left-1/2 -translate-x-1/2 text-muted-foreground/50 group-hover:text-muted-foreground transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                            <Button
                                ...
                                size="sm"
                                className={cn(
                                    "h-10 px-8 rounded-xl font-bold shadow-lg transition-transform active:scale-95 text-base text-white transition-colors duration-500",
                                    isOverBudget ? "opacity-50" : "",
                                    getButtonColor(progressPercentage)
                                )}
                            >
                                Finalizar
                            </Button>
                    ...
                    <div className="px-4 pb-1">
                        <div className="flex justify-between text-[10px] mb-1.5 font-medium uppercase tracking-wider text-muted-foreground">
                            ...
                            <span className={cn(
                                ...
                                getStatusColor(progressPercentage)
                            )}>
                        ...
                        <Progress
                            ...
                            className={cn(
                                "h-2 rounded-full bg-secondary/50 overflow-hidden transition-all duration-500",
